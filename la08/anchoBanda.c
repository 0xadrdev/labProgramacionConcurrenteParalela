  ...
  for( tam = minTam; tam <= maxTam; tam += incTam ) {
    ...
    if ( miId == 0 ) {
      ...
      for ( i = 0; i < numMensajes; i++ ) {
        MPI_Ssend(ptrWorkspace, tam, MPI_BYTE, 1, 0, MPI_COMM_WORLD);
      }

      MPI_Recv(ptrWorkspace, 0, MPI_BYTE, 1, 0, MPI_COMM_WORLD, &s);

      t2 = MPI_Wtime();

    } else if ( miId == 1 ) {
      for ( i = 0; i < numMensajes; i++ ) {
        MPI_Recv(ptrWorkspace, tam, MPI_BYTE, 0, 0, MPI_COMM_WORLD, &s);
      }

      MPI_Ssend(ptrWorkspace, 0, MPI_BYTE, 0, 0, MPI_COMM_WORLD);
    }

    // Calculo de prestaciones: tiempoTotal, tiempoPorMensajeEnMicroseg, anchoDeBandaEnMbs.
    // ... (C)
    if ( miId == 0 ) {
      tiempoTotal = t2 - t1;

      if ( tiempoTotal > 0.0 && numMensajes > 0 ) {
        double totalBytes = (double) numMensajes * (double) tam;
        double totalMB    = totalBytes / ( 1024.0 * 1024.0 );
        
        anchoDeBandaEnMbs = totalMB / tiempoTotal;
        tiempoPorMensajeEnMicroseg = ( tiempoTotal * 1000000.0 ) / (double) numMensajes;
      
      } else {
        tiempoPorMensajeEnMicroseg = 0.0;
        anchoDeBandaEnMbs          = 0.0;
      }
    }
