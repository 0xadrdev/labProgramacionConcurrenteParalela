...
int main( int argc, char *argv[] ) {
  ...
  if (miId == 0) {
      int next_idx = 0;         
      int trabajadores_activos = numProcs - 1; 

      while (trabajadores_activos > 0) {
          char buffer_peticion;
          MPI_Recv(&buffer_peticion, 1, MPI_CHAR, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &s);
          
          int trabajador_origen = s.MPI_SOURCE;

          if (next_idx < dimVectorNumeros) {
              MPI_Ssend(&vectorNumeros[next_idx], 1, MPI_LONG_LONG_INT, trabajador_origen, 55, MPI_COMM_WORLD);
              next_idx++;
          } else {
              long long dummy = 0;
              MPI_Ssend(&dummy, 1, MPI_LONG_LONG_INT, trabajador_origen, 44, MPI_COMM_WORLD);
      ...
      while(1) {
          MPI_Ssend(&peticion, 1, MPI_CHAR, 0, 0, MPI_COMM_WORLD);

          MPI_Recv(&num, 1, MPI_LONG_LONG_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &s);

          if (s.MPI_TAG == 44) {
              break; 
          } else if (s.MPI_TAG == 55) {
              if (esPrimo(num)) {
                  numPrimosParLocal++;
              }
          }
