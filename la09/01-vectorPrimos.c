...
int main( int argc, char *argv[] ) {
  ...
  if (miId == 0) {
    ...
    long long veneno = 0; 
    MPI_Send(&veneno, 1, MPI_LONG_LONG_INT, 1, 33, MPI_COMM_WORLD);
    MPI_Recv(&numPrimosPar, 1, MPI_INT, 1, MPI_ANY_TAG, MPI_COMM_WORLD, &s);
  } else if (miId == 1) { // 
      while(1) {
      ...
      MPI_Send(&numPrimosParLocal, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
  }
